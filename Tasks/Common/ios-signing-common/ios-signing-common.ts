...
export async function installCertInTemporaryKeychain(keychainPath: string, keychainPwd: string, p12CertPath: string, p12Pwd: string, privateKeyName: string, useKeychainIfExists: boolean): Promise<void> {
    ...
    //set the partition_id ACL for the private key we just imported so codesign won't prompt to use the key for signing. See
...
async function setKeyPartitionList(keychainPath: string, keychainPwd: string, privateKeyName: string) {
    // security set-key-partition-list -S apple: -s -l <privateKeyName> -k <keychainPwd> <keychainPath>
    ...
    if (privateKeyName) {
        ...
        // "security import a.p12 -k login.keychain" results in the private key with partition list "apple-tool:".
        const partitionList = 'apple:';
